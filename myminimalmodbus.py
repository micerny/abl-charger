...
def _extract_payload(
    ...
    * ASCII Mode: header (``:``) + slaveaddress byte + functioncode byte +
    ...
    if mode == MODE_ASCII:
        ...
        if response[_BYTEPOSITION_FOR_ASCII_HEADER].to_bytes(1, "big") != _ASCII_ANSWER_HEADER:
            raise InvalidResponseError(
                "Did not find header ({!r}) as start ".format(_ASCII_ANSWER_HEADER)
